...
class InferenceSystem:
    ...
    def __init__(self, model_path="global_model_final.pth", detector_path="poison_detector.pth"):
        ...
        if os.path.exists(model_path):
            ...
            raise FileNotFoundError(f"❌ CRITICAL: Model file '{model_path}' not found! Cannot proceed with random weights.")
        ...
        # CRITICAL FIX: Le détecteur a été entraîné sur le modèle base (ImageNet)
        # et non sur le modèle finetuné. Nous devons utiliser un backbone correspondant.
        self.detector_backbone = get_model(pretrained=True)
        self.poison_detector = PoisonDetector(self.detector_backbone)
    ...
    def predict_single_image(self, image_path, check_adversarial=True):
        ...
        results = {
            ...
            'adversarial_confidence': 0.0, # Will essentially be the reconstruction error
            'prediction': "BLOQUÉ",
            'confidence': 0.0,
            'all_probabilities': {}
        }
        
        # 1. GATEKEEPER: Vérifier si l'image est adversariale (Autoencoder)
        if check_adversarial:
            # detect_poison renvoie (bool_tensor, error_tensor)
            is_poisoned, error = self.poison_detector.detect_poison(image_tensor)
            
            # Correction: récupérer les valeurs scalaires
            results['is_adversarial'] = bool(is_poisoned.item())
            results['adversarial_confidence'] = float(error.item()) 
